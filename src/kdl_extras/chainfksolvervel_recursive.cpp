...
namespace KDL
{
    ChainFkSolverVel_recursive_extras::ChainFkSolverVel_recursive_extras(const Chain& _chain):
        chain(_chain)
    {
    }

    ChainFkSolverVel_recursive_extras::~ChainFkSolverVel_recursive_extras()
    {
    }

    int ChainFkSolverVel_recursive_extras::JntToCart(const JntArrayVel& in,FrameVel& out,int seg_nr)
    {
    ...
    int ChainFkSolverVel_recursive_extras::JntToCart(const JntArrayVel& in,std::vector<FrameVel>& out,int seg_nr)
    {
        unsigned int segmentNr;
        if(seg_nr<0)
            segmentNr=chain.getNrOfSegments();
